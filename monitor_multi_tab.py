...
class TabRutasWidget(QWidget):
    ...
    def crear_tabla(self):
        ...
        header.setSectionsMovable(True)  # Permite mover columnas
        header.setSectionResizeMode(QHeaderView.Interactive)  # Permite redimensionar manualmente
    ...
    def actualizar_fila_especifica(self, fila):
        ...
        if fila < len(self.rutas):
            ...
            conteo, detalles = self.contar_archivos_ruta(ruta)
    ...
    def contar_archivos_ruta(self, ruta):
        """Contar archivos en una ruta específica"""
        try:
            ruta_path = Path(ruta)
            ...
            return len(archivos), archivos[:15]
    ...
    def eliminar_ruta(self, ruta=None):
        ...
        self.tabla.removeRow(self.rutas.index(ruta) if ruta in self.rutas else row)
...
class ContadorArchivosApp(QMainWindow):
    ...
    def inicializar_interfaz(self):
        ...
        self.setWindowTitle("Monitor de Archivos - Sistema Multi-Pestañas")
        ...
        # Panel principal con splitter horizontal
    ...
    def crear_panel_principal(self, layout_principal):
        ...
        # Panel izquierdo (pestañas principales)
        ...
        # Configurar menú contextual para las pestañas
        self.tab_widget.setContextMenuPolicy(Qt.CustomContextMenu)
        self.tab_widget.customContextMenuRequested.connect(self.mostrar_menu_contexto_pestanas)
        
        layout_izquierdo.addWidget(self.tab_widget)
        
        # Panel derecho (detalles y logs)
        ...
        layout_ruta.addWidget(self.lbl_ruta_clicable, 1)  # Factor de expansión 1
        ...
        self.btn_toggle_panel.clicked.connect(self.toggle_panel_derecho)
        self.btn_toggle_panel.setToolTip("Ocultar/Mostrar panel derecho")
        
        layout_derecho.addWidget(self.tab_detalles_logs)
        
        # Añadir widgets al splitter
        self.splitter_principal.addWidget(panel_izquierdo)
        self.splitter_principal.addWidget(self.panel_derecho)
        
        # Establecer proporciones iniciales
        self.splitter_principal.setSizes([1000, 400])
        
        layout_principal.addWidget(self.splitter_principal, 1)  # Factor de expansión 1
    ...
    def mostrar_menu_contexto_pestanas(self, pos):
        """Mostrar menú contextual para las pestañas"""
        ...
        if tab_index >= 0:
            ...
            if widget:
                ...
                action_renombrar = menu.addAction("📝 Renombrar Pestaña")
                menu.addSeparator()
                action_eliminar = menu.addAction("🗑️ Eliminar Pestaña")
                menu.addSeparator()
                action_recargar = menu.addAction("🔄 Recargar Pestaña")
                action_guardar = menu.addAction("💾 Guardar Pestaña")
                
                action = menu.exec_(self.tab_widget.mapToGlobal(pos))
                
                if action == action_renombrar:
                    self.renombrar_tab_dialogo(widget.nombre)
                elif action == action_eliminar:
                    self.tab_widget.setCurrentIndex(tab_index)
                    self.eliminar_tab_actual()
                elif action == action_recargar:
                    self.tab_widget.setCurrentIndex(tab_index)
                    self.recargar_tab_actual()
                elif action == action_guardar:
                    self.tab_widget.setCurrentIndex(tab_index)
                    self.guardar_tab_actual()
    ...
    def toggle_panel_derecho(self):
        ...
        if self.panel_derecho_visible:
            ...
            # Restaurar tamaño
            sizes = self.splitter_principal.sizes()
            if sum(sizes) > 0:
                total = sum(sizes)
                left_size = int(total * 0.7)
                right_size = total - left_size
                self.splitter_principal.setSizes([left_size, right_size])
        else:
            # Guardar tamaño antes de ocultar
            sizes = self.splitter_principal.sizes()
            if len(sizes) == 2 and sizes[1] > 0:
                self.ancho_panel_derecho = sizes[1]
            ...
            sizes = self.splitter_principal.sizes()
            if sum(sizes) > 0:
                self.splitter_principal.setSizes([sum(sizes), 0])
    ...
    def eliminar_tab_actual(self):
        ...
        respuesta = QMessageBox.question(
            ...
            f"¿Está seguro de que desea eliminar la pestaña '{self.tab_actual}'?\n\n"
        ...
        if respuesta == QMessageBox.Yes:
            ...
            if self.tab_actual in self.tabs:
                del self.tabs[self.tab_actual]
            ...
            self.agregar_log(f"🗑️ Pestaña eliminada")
    ...
    def renombrar_tab_dialogo(self, nombre_actual):
        ...
        if ok and nuevo_nombre and nuevo_nombre != nombre_actual:
            ...
            if nombre_actual in self.tabs:
                ...
                # Renombrar en el widget
                index = self.tab_widget.indexOf(tab)
                if index >= 0:
                    self.tab_widget.setTabText(index, nuevo_nombre)
    ...
    def on_tab_change(self, index):
        ...
        if index >= 0:
            ...
            if widget:
                self.tab_actual = widget.nombre
                # Mostrar detalles de la selección actual en la nueva pestaña
    ...
    def actualizar_info_tabs(self):
        ...
        self.lbl_total_tabs.setText(f"Pestañas: {len(self.tabs)}")
    ...
    def iniciar_monitoreo(self):
        ...
        if not self.hilo_activo:
            ...
            self.timer.start(self.intervalo_actualizacion * 1000)  # Convertir a milisegundos
            self.agregar_log("▶ Monitoreo iniciado para todas las pestañas")
            self.lbl_estado.setText(f"Estado: Monitoreando ({len(self.tabs)} pestañas)")
    ...
    def mostrar_detalles_actuales(self):
        ...
        self.lbl_ruta_clicable.setStyleSheet("color: #0645AD;")
        self.btn_abrir_directorio.setEnabled(True)
        
        # Actualizar detalles
        info = f"Pestaña: {datos['nombre_tab']}\n"
        info += f"Ruta: {ruta}\n\n"
        info += f"Archivos actuales: {datos['actual']}\n"
        info += f"Archivos anteriores: {datos['anterior']}\n"
        info += f"Cambio: {datos['cambio']}\n"
        info += f"Estado: {datos['estado']}\n"
        info += f"Última verificación: {datos['ultima_verificacion']}\n"
        info += f"Archivos recursivos: {tab.contar_archivos_recursivo(ruta)}\n"
        info += "\n" + "="*40 + "\n"
        
        if datos['detalles'] and isinstance(datos['detalles'], list):
            info += f"Primeros {len(datos['detalles'])} archivos:\n"
            for archivo in datos['detalles']:
                if len(archivo) > 30:
                    info += f"  • ...{archivo[-27:]}\n"
                else:
                    info += f"  • {archivo}\n"
        
        self.txt_detalles.setText(info)
    ...
    def cargar_configuracion(self):
        ...
        if os.path.exists(config_file):
            try:
                ...
                # Aplicar configuración del panel derecho
    ...
    def mostrar_acerca_de(self):
        ...
        QMessageBox.about(self, "Acerca de",
            f"""<b>Monitor de Archivos - Sistema Multi-Pestañas</b><br><br>
            Versión: 2.0<br>
    ...
    def mostrar_manual(self):
        ...
        manual_text = """
        <h2>Manual de Usuario - Monitor de Archivos</h2>
        
        <h3>📋 Funcionalidades Principales:</h3>
        <ul>
        <li><b>Pestañas múltiples:</b> Crea y gestiona diferentes grupos de rutas</li>
        <li><b>Monitoreo automático:</b> Cuenta archivos en intervalos configurables</li>
        <li><b>Configuración persistente:</b> Guarda automáticamente todos los ajustes</li>
        <li><b>Interfaz personalizable:</b> Redimensiona columnas y paneles</li>
        </ul>
        
        <h3>🎯 Uso Básico:</h3>
        <ol>
        <li><b>Crear pestaña:</b> Menú Archivo → Nueva Pestaña</li>
        ...
        <h3>📝 Consejos:</h3>
        <ul>
        <li>Haz clic derecho en las pestañas para opciones adicionales</li>
        <li>Haz clic derecho en los encabezados de columnas para ajustar</li>
        <li>Arrastra los divisores para cambiar tamaños de paneles</li>
        <li>Doble clic en una ruta para abrir el directorio</li>
        </ul>
        """
    ...
    def exportar_todo_estructurado(self, archivo):
        ...
        with open(archivo, 'w', encoding='utf-8') as f:
            ...
            f.write("=" * 80 + "\n")
            f.write("INFORME COMPLETO DE MONITOREO MULTI-PESTAÑAS\n")
            f.write("=" * 80 + "\n\n")
            ...
            f.write(f"Generado por Monitor de Archivos Multi-Pestañas v2.0\n")
            f.write("=" * 80 + "\n")
